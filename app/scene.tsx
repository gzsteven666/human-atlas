...
import {anatomyName,type Locale} from './i18n';
...
export default function AnatomyScene({atlas,locale,state,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),latestLocale=useRef(locale);
 latest.current=state;select.current=onSelect;latestLocale.current=locale;
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError(locale==='zh'?'此浏览器无法启动三维查看器，请使用支持 WebGL 的浏览器。':'This browser could not start the 3D viewer. Please try a browser with WebGL enabled.');return;}
  ...
  renderer.domElement.setAttribute('aria-label',locale==='zh'?'交互式人体解剖模型。拖动旋转，双指或滚轮缩放，点击结构查看详情。':'Interactive human anatomy. Drag to orbit, pinch or scroll to zoom, and tap a structure to inspect it.');
  ...
  const materialFor=(system:string)=>{
   ...
   m.onBeforeCompile=shader=>{
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.545, 0.361, 0.965), partSelected * 0.95);');
    shader.fragmentShader=shader.fragmentShader.replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\ntotalEmissiveRadiance += vec3(0.20, 0.06, 0.48) * partSelected;');
  ...
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:(locale==='zh'?'无法加载解剖模型。':'Could not load the anatomy.'));}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError(locale==='zh'?'三维会话已被设备暂停，请重新加载。':'The 3D session was paused by your device. Reload to continue.');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
